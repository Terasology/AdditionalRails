...
public class WrenchOnewayAction extends BaseComponentSystem {
    ...
    public void onRailFlipAction(ActivateEvent event, EntityRef item) {
        ...
        RailBlockFamily originalFamily = (RailBlockFamily) blockManager.getBlockFamily("AdditionalRails" +
                ":OnewayBoosterRail");
        RailBlockFamily invertedFamily = (RailBlockFamily) blockManager.getBlockFamily("AdditionalRails" +
                ":OnewayBoosterRailInverted");

        //Using the Block from BlockComponents directly causes rails to return to their original orientation when
        // their orientation was changed because of being connected to a T or intersection.
