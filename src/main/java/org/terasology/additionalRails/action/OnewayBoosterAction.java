...
public class OnewayBoosterAction extends BaseComponentSystem implements UpdateSubscriberSystem {
    ...
    private final Set<RailCart> entities = Sets.newHashSet(); //The set of all pairs of a tile of OnewayBoosterRail
    // and a vehicle on it
    ...
    public void update(float delta) {
        ...
        for (RailCart rc : entities) {
            ...
            if (blockMappingComponent.s1 == Side.TOP || blockMappingComponent.s2 == Side.TOP) {
                railDirection.addY(1); //Every original slope rail points upward and inverted ones downward, so the y
