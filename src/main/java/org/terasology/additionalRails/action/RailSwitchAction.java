...
public class RailSwitchAction extends BaseComponentSystem {
     ...
     *
     * @param event OnChangedComponent event raised by signal modyfying {@link
     *         org.terasology.signalling.components.SignalConsumerStatusComponent}
     * @param entity The entity related to railSwitchSignal
     */
    @ReceiveEvent(components = {BlockComponent.class, RailSwitchSignalComponent.class,
            SignalConsumerStatusComponent.class})
     ...
     * @param inverted boolean which is false if the rail juntion should be normal, or true, if the rail juntion
     *         should be inverted
     */
            ...
            if (SideBitFlag.getSides(connections).size() == 3) {
                if (block.getBlockFamily() == railFamily || block.getBlockFamily() == invertFamily) {
                    blockEntityRegistry.setBlockForceUpdateEntity(railLocation, inverted ?
                            invertFamily.getBlockByConnection(connections) :
                            railFamily.getBlockByConnection(connections));
                }
            }
        }
    }
