...
public class TunnelBoreCartAction extends BaseComponentSystem {
    ...
    public void onLayTrack(LayTrackEvent event, EntityRef cart, TrackLayerCartComponent comp,
                           TunnelBoreCartComponent boreComp) {
        ...
        Vector3i cartLoc =
                cart.getComponent(PathFollowerComponent.class).segmentMeta.association.getComponent(BlockComponent.class).position;
        ...
        Prefab damageType = boreDrill == EntityRef.NULL ?
                entityManager.create("engine:physicalDamage").getParentPrefab() :
